...
fun Counter() {
    ...
    Row(
        ...
        Button(onClick = { count.intValue = (count.intValue - 1).coerceAtLeast(0) }) {
            Text(text = stringResource(R.string.btn_decrement))
        }

        Text(
            text = stringResource(R.string.lbl_count, count.intValue),
    ...
    LaunchedEffect(Unit) {
        snapshotFlow { count.intValue }.collect {
